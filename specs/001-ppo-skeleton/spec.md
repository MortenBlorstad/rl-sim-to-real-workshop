...
- **FR-027**: When run as a script, the skeleton MUST verify on exit that no printed loss was NaN and that the last printed loss is strictly less than the first printed loss; if either check fails it MUST print an actionable error message naming the failure. This is the script-level equivalent of the smoke test.
